...
def add_event_owner(request):
    ...
    error = ""
    category = Category.objects.all()
    us=request.user.first_name+request.user.last_name
    if request.method == 'POST':
        en = us
        i = request.FILES['image']
        cat = request.POST['category']
        ...
        try:
            newobj = OwnerEvent.objects.create(eventname=en, image=i, category=cat, description=des, startdate=sd, enddate=ed,
                                 venue=ve, entryprice=ep)
            error = "no"
        except:
            error = "yes"

    return render(request, "add_event_owner.html", locals())
...
def edit_event_owner(request, pid):
    if not request.user.is_authenticated:
        return redirect('Login')
    error = ""
    category = Category.objects.all()
    event = OwnerEvent.objects.get(id=pid)
    if request.method == 'POST':
       #en = request.POST['ename']
        ...
        try:
            ...
            pass
    return render(request, "edit_event_owner.html", locals())
...
def view_event_owner(request):
    if not request.user.is_authenticated:
        return redirect('Login')
    event = OwnerEvent.objects.all()
    d = {'event': event}
    return render(request, "view_event_owner.html", d)
...
def delete_event(request, pid):
    ...
    try:
        a= OwnerEvent.objects.get(id=pid)
        a.delete
    except:
        pass
    apoint.delete()
...
def delete_event_owner(request, pid):
    if not request.user.is_authenticated:
        return redirect('Login')
    apoint = OwnerEvent.objects.get(id=pid)
    try:
        a=Event.objects.get(id=pid)
        a.delete()
    except:
        pass

    apoint.delete()
    return redirect('view_event_owner')
...
def booking_request_owner(request):
    if not request.user.is_authenticated:
        return redirect('Login')
    b = OwnerEvent.objects.filter(status="pending")
    return render(request, "booking_request_owner.html", locals())
